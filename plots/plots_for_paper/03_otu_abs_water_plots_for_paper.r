...
a<-ggplot(data=Gallaecimonas_abs_water_plot, aes(x=days-69,y=value,group=nucleic_acid,lty=nucleic_acid))+ 
	...
	annotate("segment", x = -8.0, xend = 18, y = 4300000, yend = 4300000, colour = "grey40", size=2, alpha=1,lineend = "butt")+
	annotate("segment", x = 18.0, xend = 18, y = 4440000, yend = 4160000, colour = "grey40", size=2, alpha=1,lineend = "butt")+
	annotate("segment", x = -8.0, xend = -8, y = 4440000, yend = 4160000, colour = "grey40", size=2, alpha=1,lineend = "butt")+
	geom_vline(aes(xintercept=0),linetype="dashed", size=1.2)+
	geom_point(data=subset(Gallaecimonas_abs_water_plot,treatment=="control"),aes(colour=treatment),alpha=.6)+
	stat_summary(data=subset(Gallaecimonas_abs_water_plot,treatment=="control"),aes(colour=treatment),fun.y="mean", geom="line", size=1.5,alpha=.6)+
	stat_summary(data=subset(Gallaecimonas_abs_water_plot,treatment=="glyph"),aes(colour=treatment),fun.y="mean", geom="line", size=3)+
	...
	scale_colour_manual(values=c("glyph"="black","control"="grey40"),
						name="Microcosm  ",
...
#####then the Pseudomonas plot
Pseudomonas_abs_water_plot<-subset(abs_abu_molten_tax, habitat == "water" & grepl('Pseudomonas',genus) & days > 40)
species_title_Pseudomonas<-expression(paste(,italic("Pseudomonas")," sp."))
Pseudomonas_abs_water_plot$treatment2<-factor(Pseudomonas_abs_water_plot$treatment,labels=c("Control","Treatment"))

b<-ggplot(data=Pseudomonas_abs_water_plot, aes(x=days-69,y=value,group=nucleic_acid,lty=nucleic_acid))+ 
	#coord_cartesian(ylim = c(0, 80000))+
	annotate("segment", x = -8.0, xend = 18, y = 11500000, yend = 11500000, colour = "grey40", size=2, alpha=1,lineend = "butt")+
	annotate("segment", x = 18.0, xend = 18, y = 11900000, yend = 11100000, colour = "grey40", size=2, alpha=1,lineend = "butt")+
	annotate("segment", x = -8.0, xend = -8, y = 11900000, yend = 11100000, colour = "grey40", size=2, alpha=1,lineend = "butt")+
	geom_vline(aes(xintercept=0),linetype="dashed", size=1.2)+
	geom_point(data=subset(Pseudomonas_abs_water_plot,treatment=="control"),aes(colour=treatment),alpha=.6)+
	stat_summary(data=subset(Pseudomonas_abs_water_plot,treatment=="control"),aes(colour=treatment),fun.y="mean", geom="line", size=1.5,alpha=.6)+
	stat_summary(data=subset(Pseudomonas_abs_water_plot,treatment=="glyph"),aes(colour=treatment),fun.y="mean", geom="line", size=3)+
	geom_point(data=subset(Pseudomonas_abs_water_plot,treatment=="glyph"),aes(colour=treatment))+
	scale_linetype_manual(values=c("dna"=1,"cdna"=6),
						name="Nucleic acid  ",
						breaks=c("cdna","dna"),
						labels=c("16S rRNA","16S rRNA gene"))+
...
grid.arrange(a, b, nrow = 1, bottom=textGrob(expression(bold("Days")),gp=gpar(fontsize=22)),left=textGrob(c,rot=90,gp=gpar(fontsize=22)))

ggsave(file="fig_03_OTU_plots_paper_Pseudomonas_Gallaeci.png", width=20, height=8.75,
